...
class DispatchItem(OleItem):
    ...
    def __init__(
        ...
        if typeinfo:
            self.Build(typeinfo, attr, bForUser, iCreateEnums)
    ...
    def _AddFunc_(self, typeinfo, fdesc, bForUser, iCreateEnums):
    ...
    def _AddVar_(self, typeinfo, vardesc, bForUser, iCreateEnums):
    ...
    def Build(
        ...
        for j in range(attr[6]):
            ...
            self._AddFunc_(typeinfo, fdesc, bForUser, iCreateEnums)

        # Loop over all variables (ie, properties)
        for j in range(attr[7]):
            fdesc = typeinfo.GetVarDesc(j)
            self._AddVar_(typeinfo, fdesc, bForUser, iCreateEnums)
    ...
    def MakeDispatchFuncMethod(
        ...
        out_params = [
            flag
            for flag in param_flags
            if flag & (pythoncom.PARAMFLAG_FOUT) != 0
        ]
        ...
        if ((resultType is not None) and resultType != "None"):
            return_type_list.append((resultType,fdesc[8][5] == pythoncom.TKIND_ENUM, fdesc[8][0], _MakeTypeHint(entry.resultDocumentation, fdesc[8][0] & ~0x2000 )))

        for i in range(0, len(fdesc[2])):
            if (fdesc[2][i][1] & pythoncom.PARAMFLAG_FOUT) != 0:
                return_type_list.append((_MakeTypeHint(fdesc[2][i][4], fdesc[2][i][0]),fdesc[2][i][5] == pythoncom.TKIND_ENUM, fdesc[2][i][0], _MakeTypeHint(fdesc[2][i][4], fdesc[2][i][0] & ~0x2000)))
        ...
        if bTypeHints:
            if len(return_type_list) == 0:
                ...
                linePostfix = ") -> " + return_type_list[0][0] + ":"
            else:
                linePostfix = ") -> Tuple["
                first = True
                for t in return_type_list:
                    if first:
                        first = False
                    else:
                        linePostfix += ", "
        ...
        # simple result pre/postfix, used when there are no BY_REF arguments
        if (iCreateEnums & ENUMS_CREATE_ENUM_CLASSES) != 0:
            if fdesc[8][5] == pythoncom.TKIND_ENUM:
                # here we can encode the enum-name as type
                # resclsid = _MakeResultTypeCreator(entry.resultDocumentation, fdesc[8][0])
                if (fdesc[8][0] & 0x2000) != 0:
                    result_prefix = " [" + fdesc[8][4][0] + "(x) for x in "  
                    result_postfix = "]"
                else:
        ...
        if len(bad_params) == 0 and len(retDesc) == 2 and retDesc[1] == 0:
            ...
            # else s remains None
        if s is None:
            # check, if we have to create enums!
            if len([t for t in return_type_list if t[1]]) != 0:
                if len(return_type_list) == 1:
                    if (return_type_list[0][2] & 0x2000) != 0:
                        result_prefix = " [" + return_type_list[0][3] + "(x) for x in "  
                        result_postfix = "]"
                    else:
                        result_prefix = return_type_list[0][0] + "("
                        result_postfix = ")"
                    ...
                    # s = "%s\treturn ret= self._ApplyTypes_(%d, %s, %s, %s, %s, %s%s)\n" % (
                    #     linePrefix,
                    #     id,
                    #     fdesc[4],
                    #     retDesc,
...
def _ResolveType(typerepr, itypeinfo, iCreateEnums):
    ...
    if isinstance(typerepr, tuple):
        ...
        if indir_vt == pythoncom.VT_CARRAY:  # runtime has no support for this yet.
            ...
            return pythoncom.VT_CARRAY, None, None, -1
        if indir_vt == pythoncom.VT_USERDEFINED:
            try:
                ...
                if details.hresult in [
                    ...
                    return pythoncom.VT_UNKNOWN, None, None, -1
                raise
